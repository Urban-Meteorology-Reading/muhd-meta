...
for process_site in sites_to_process:
    ...
    if os.path.exists(meta_out_dir) == False:
    ...
    dates_df = dates_df.rename(columns = {'instId' : 'Instrument ID', 'instSerial' : 'Instrument serial', 
                                        'suffix' : 'Letter', 'type' : 'Type', 'friendlyName' : 'Deployment name', 'startDate' : 'Start date', 
                                        'endDate' : 'End date'})
    ...
    if os.path.exists(dates_out_dir) == False:
        os.makedirs(dates_out_dir)
    ...
    if os.path.exists(pos_out_dir) == False:
    ...
    if os.path.exists(raw_out_dir) == False:
    ...
    if any([inst_id in scint_inst_ids for inst_id in site_inst_ids]):
        ...
        if os.path.exists(scint_out_dir) == False:
            os.makedirs(scint_out_dir)
    ...
    if any([inst_id in indoor_inst_ids for inst_id in site_inst_ids]):
        ...
        if os.path.exists(indoor_out_dir) == False:
            os.makedirs(indoor_out_dir)
