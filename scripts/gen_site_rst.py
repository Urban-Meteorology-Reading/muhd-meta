...
#%%
#initiate site file
site_file_path = os.path.join('..', 'source', 'LUMA', 'sites',)
...
with open(site_path, 'w+') as site_file:
    ...
    site_file.write(f"   :header-rows: 2\n\n")
    #scint
    if os.path.exists(os.path.join('..', 'source', 'LUMA', 'sites', "deployments", "scint_deployments", f"{site}_scint_deployments.csv")):
        ...
        site_file.write(f"   :header-rows: 2\n\n")
    #indoor
    if os.path.exists(os.path.join('..', 'source', 'LUMA', 'sites', "deployments", "indoor_deployments", f"{site}_indoor_deployments.csv")):
    ...
    if os.path.exists(os.path.join('..', 'source', 'LUMA', 'sites', 'photos', site, f'{site}_photo_ref.csv')):
        photo_ref = pd.read_csv(os.path.join('..', 'source', 'LUMA', 'sites', 'photos', site, f'{site}_photo_ref.csv'))
        photo_ref.apply(lambda x: write_figures(x, site, site_file), axis = 1)

    #supplementary info
    site_file.write(f"Supplementary information\n{'#'*25}\n\n")
    if os.path.exists(os.path.join('..', 'source', 'LUMA', 'sites', 'supplementary_info', 'supplementary_info.csv')):
        sup_info = pd.read_csv(os.path.join('..', 'source', 'LUMA', 'sites', 'supplementary_info', 'supplementary_info.csv'))
    ...
    if os.path.exists(os.path.join('..', 'source', 'LUMA', 'sites', 'references', 'references.csv')):
